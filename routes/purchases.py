...
def _recalculate_menu_costs(db):
    """Every time I log a purchase, I recalculate menu item costs from the latest prices.
    This keeps the profit margins accurate without me having to update them manually."""
    items = db.execute("SELECT name, price, quantity, unit FROM purchase_items").fetchall()

    # starting defaults so the math doesn't break if a category has no purchases yet
    ...
    e2 = costs['egg'] * 2
    e3 = costs['egg'] * 3
    c70 = costs['chicken_per_lb'] * 0.0795   # ~70g serving
    c200 = costs['chicken_per_lb'] * 0.227   # ~200g for Chicken 65
    rice = costs['rice_per_lb'] * (1 / 2.75) # ~1lb makes 2.75 portions
...
def add_purchase_item():
    """Logs the purchase and immediately recalculates menu costs from the new price."""
    data = request.json
    db = get_db()
    db.execute(
